...
function Write-LaunchVbs {
    ...
    # CreateProcess()'s closedmesh.exe with no console window, then
    # blocks until it exits. Why bother:
    #
    # closedmesh.exe is a console-subsystem binary (Cargo default for
    # `[[bin]]`). When a Scheduled Task with LogonType=Interactive
    # runs it, Windows always allocates a console — and on Windows 11
    # the user's default terminal handler (Windows Terminal) opens a
    # visible tab titled with the binary's path. The bundled runtime
    # also paints a TUI status board ("Running llama.cpp instances",
    # "Mesh events…") which makes the desktop install look like it
    # spawned a stray dev terminal on every login.
    #
    # wscript.exe is a Windows-subsystem host: launching the runtime
    # through `WScript.Shell.Run "...", 0, True` opens it with no
    # window AND keeps wscript alive until closedmesh exits, so
