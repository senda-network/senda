...
# no console window, then blocks until it exits.
#
# Why not point the Scheduled Task at closedmesh.exe directly? It's a
# console-subsystem binary, and a Scheduled Task with
# LogonType=Interactive always allocates a console for it — which on
# Windows 11 means the user's default terminal handler (Windows
# Terminal) opens a visible tab titled with the binary's path on
# every login. The runtime also paints a TUI status board ("Running
# llama.cpp instances", "Mesh events…") that makes the desktop
# install look like it spawned a stray dev terminal.
#
# wscript.exe is a Windows-subsystem host: launching the runtime
# through `WScript.Shell.Run "...", 0, True` opens it with no window
# AND keeps wscript alive until closedmesh exits, so
# Stop-ScheduledTask still has something to terminate.
$vbsPath = Join-Path $installDir 'closedmesh-launch.vbs'
$escBin  = $BinPath.Replace('"', '""')
$escArgs = $ArgString.Replace('"', '""')
$vbs = @"
...
' 0 = SW_HIDE (no window), True = wait for the child to exit so the
' Scheduled Task framework treats the task as running while the
' runtime is alive (and Stop-ScheduledTask actually terminates it).
sh.Run """${escBin}"" ${escArgs}", 0, True
Set sh = Nothing
"@
...
/// from a pre-VBS-launcher version of the desktop app).
///
/// We can't just read the task's Action.Arguments because as of the
/// VBS-launcher refactor the task runs `wscript.exe //B //Nologo
/// "<vbs>"` — the actual closedmesh args live inside the `.vbs`. So
/// we read the .vbs sibling of `bin` and pull the args out of its
/// `sh.Run` line.
///
...
fn current_windows_task_args(bin: &std::path::Path) -> Option<String> {
    ...
    // Grab the contents of the `sh.Run "..."` invocation. Format we
    // emit: `sh.Run """<bin>"" <args>", 0, True`. After PowerShell
    // here-string interpolation each `""` collapses to a single `"`,
    // so the `.vbs` actually on disk looks like
    //   sh.Run """C:\path\closedmesh.exe"" serve --auto ...", 0, True
    // We strip the leading bin path (with its surrounding `""…""`)
    // and return the rest as the canonical arg string.
    let prefix = "sh.Run \"\"\"";
    let line = contents.lines().find(|l| l.contains(prefix))?;
    let after_prefix = line.split_once(prefix)?.1;
    // After `<bin>` comes `""<space>...`, then `", 0, True` at the end.
    let close_idx = after_prefix.find("\"\"")?;
    let after_bin = &after_prefix[close_idx + 2..]; // skip the `""`
    let trimmed = after_bin.trim_start();
    let end = trimmed.rfind("\", 0,")?;
