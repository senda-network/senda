...
- **`FALLBACK_JOIN_TOKEN` in `mesh.rs`** only needs updating when the entry node container's iroh identity actually rotates — which is NO LONGER every restart (the keypair is persisted at `/opt/closedmesh-data/key`; verified stable across restart Jun 1 2026). Real rotation triggers: wiping `/opt/closedmesh-data` (deletes `key`), OR a public→private / private→public flip (the runtime auto-rotates in this case — see logs for `Previous run was public — rotating identity`). A plain `systemctl restart` / image redeploy keeps the same `node_id`, so contributors stay connected.
