...
const explicit = process.env.SENDA_BIN ?? process.env.FORGEMESH_BIN;
...
// dropped a binary at AppData\Local\senda\bin\senda.exe.
...
const candidates = [
  ...
  isWindows
    ? path.join(homedir(), "AppData", "Local", "senda", "bin", "senda.exe")
    : null,
  ...
  isWindows
    ? path.join(homedir(), ".local", "bin", "senda.exe")
    : path.join(homedir(), ".local", "bin", "senda"),
  // macOS: Homebrew (Apple Silicon and Intel respectively).
  isWindows ? null : "/opt/homebrew/bin/senda",
  isWindows ? null : "/usr/local/bin/senda",
...
/**
 * Locate the senda runtime binary on disk.
 *
...
// fail on Windows even when senda.exe is perfectly executable.
...
export async function findSendaBin(): Promise<string | null> {
  if (cachedBin) {
    try {
  ...
  return new Promise((resolve) => {
    ...
    try {
      child = spawn(bin, args, {
        ...
        // like `senda.exe`, `schtasks.exe`, or `tar.exe`, Windows
...
// systemd unit, set up by `senda service install`) does the
...
// targeting %LOCALAPPDATA%\senda\logs. Pre-0.1.68 this constant
...
export const LOG_PATHS =
  process.platform === "win32"
    ...
    : process.platform === "darwin"
      ...
      : {
          // Linux: matches the systemd unit that senda-llm's
...
/**
 * The `senda service start` command can emit a multi-section status
 * dump after an error (the "senda ┌ Running …" block, or the legacy
...
export function extractStartError(raw: string): string {
  ...
  // The status dump starts with a bare binary-name line ("senda" on
